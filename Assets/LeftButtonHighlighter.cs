...
public class LeftButtonHighlighter : MonoBehaviour
{
    ...
    private readonly Vector2 centerLocalXY = new Vector2(0, 120f);
    private readonly Vector2 size = new Vector2(600f, 220f);
    private readonly string sceneName = "PhaseMenu_Left";

    void Start()
    {
        buttonImage = GameObject.Find("LeftHandButton")?.GetComponent<Image>();
    }
    ...
    void Update()
    {
        ...
        {
            Debug.LogWarning("[LeftHighlighter] 🚫 Missing references or scene loading.");
            return;
        }
        ...
        if (buttonImage == null)
        {
            Debug.LogWarning("[LeftHighlighter] ❌ Button image not found.");
            return;
        }
        ...
        if (triggerPressed)
        {
            if (isInside)
            {
                Debug.Log($"[LeftHighlighter] ✅ Triggered {sceneName}");
                sceneLoading = true;
                SceneManager.LoadScene(sceneName);
            }
            else
            {
                Debug.Log("[LeftHighlighter] ❌ Trigger pressed but not inside button.");
            }
        }
    }
}
